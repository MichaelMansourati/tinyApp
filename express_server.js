...
function checkLogin(database, userEmail, userPassword) {
  ...
  let foundUser = usersArr.find(user => {
    return (
      ...
      bcrypt.compareSync(userPassword, user.hashPassword)
    );
  });
  if (foundUser) {
    return foundUser;
...
app.get("/urls", (req, res) => {
  ...
  if (currentUser) {
    ...
    res
      .status(401)
      .send(
        "<p>You must be logged in to access this function. <a href='/login'>login here</a></p>"
      );
  }
});
...
app.get("/urls/:id", (req, res) => {
  ...
  if (urlDatabase[currentUser.id][req.params.id]) {
    ...
    res
      .status(403)
      .send(
        "This short url belongs to another user and may only be modified them."
      );
  } else {
...
app.post("/login", (req, res) => {
  ...
  if (userEmail && userPassword) {
    ...
    res
      .status(403)
      .send(
        "Please check to make sure all fields have been filled out appropriately."
      );
  }
});
  ...
  if (email && hashPassword) {
    if (validateEmail(users, email) === true) {
      ...
      res
        .status(400)
        .send(
          "The email address you have chosen is already in use. Please choose another."
        );
    }
  } else {
    res
